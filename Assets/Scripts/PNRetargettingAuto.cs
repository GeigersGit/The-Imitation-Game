...
public class PNRetargettingAuto : MonoBehaviour
{
    ...
    void Update()
    {
        if (Application.loadedLevelName == "fight" && retrieveRobot) {
            mainPNModel = GameObject.Find("Robot Rigged").transform;
            NeckJoint = GameObject.Find("Robot_Neck").transform;
            HeadJoint = GameObject.Find("Robot_Head").transform;
            AssignPrefixJoints();
            retrieveRobot = false;
        }
